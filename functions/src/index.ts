...
exports.exportOrderData = functions.region(region).firestore.document('users/{id}').onCreate(async (snapshot, _) => {
    const docData = snapshot.data();

    const date = docData.dateCreated.toDate().toLocaleString('en-US', { datestyle: 'long', timestyle: 'long'});
    ...
    await exportUsers(date, fullName, isFullyLoggedIn, shirtSize, school,
